...
# 2. Creating a Windows Virtual Machine with Quickemu

A virtual machine (VM) lets you run Windows inside another operating system without affecting your main computer.

One simple way to do this on Linux is **Quickemu**, which uses **QEMU** in the background.

---

## Quickget

`quickget` downloads the operating system image (ISO).
...
Once the ISO is downloaded, Quickemu creates and starts the virtual machine.

Example:

```bash
quickemu --vm Windows_11.conf
```
...
Install using an AUR helper like:
...
Quickemu supports the **SPICE** display protocol.
